...
class CustomerPortal(portal.CustomerPortal):
    ...
    def upload_portal_documents(self, access_token=None):
        ...
        accessible_directories = request.env['documents.folders'].search([
            ('upload_access', 'in', [user.id])
        ])
        directories = [{'id': dir.id, 'name': dir.name} for dir in accessible_directories]
    ...
    @http.route(['/my/documents/upload_portal_documents/upload'], type='http', auth="public", methods=['POST'],
                website=True)
    def portal_documents_upload(self, **post):
        user = request.env.user
        user_sudo = request.env['res.users'].sudo().browse(user.id)
        folder_id = post.get('folder_selection', 'nofolder')
        try:
            folder = None
            if folder_id != 'nofolder':
                folder = request.env['documents.folders'].sudo().browse(int(folder_id))
                if not folder.exists():
                    return json.dumps({'error': _('Selected folder does not exist.')})
            ...
            attach_vals = {
                ...
                'res_id': user_sudo.partner_id.id if user_sudo else False,
                'partner_id': user_sudo.partner_id.id if user_sudo else False,
            ...
            return json.dumps(
                {'Error': _('Sorry, but the uploaded file is either invalid or too large! Reason: %s') % (e,)})

        return request.redirect('/my/documents')

    @http.route(['/my/documents', '/my/documents/page/<int:page>'], type='http', auth="user", website=True)
    def portal_my_documents(self, page=1, date_begin=None, date_end=None, sortby=None, filterby=None, search=None,
        ...
        domain = [
            ...
            ('folder_id.read_access', 'in', [user.id])
        ]
        ...
        searchbar_filters = {
            ...
            'shared_documents': {'label': _('Shared Documents'),
                                 'domain': [('shared_user_ids', 'in', [user.id]), ('create_uid', 'not in', [user.id])]},
        }
        ...
        if groupby == 'none':
            ...
            grouped_documents = [Attachment.sudo().concat(*g) for k, g in
                                 groupbyelem(documents, itemgetter('folder_id'))]
