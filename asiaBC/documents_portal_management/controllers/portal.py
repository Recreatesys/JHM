...
class CustomerPortal(portal.CustomerPortal):
    ...
    @http.route(['/my/documents/upload_portal_documents/upload'], type='http', auth="user", methods=['POST'], website=True)
    def portal_documents_upload(self, **post):
        user_sudo = request.env.user.sudo()
        portal_directory = request.env.ref('documents_portal_management.portal_folder', raise_if_not_found=False)
        
        try:
            uploaded_files = request.httprequest.files.getlist('upload_documents')
