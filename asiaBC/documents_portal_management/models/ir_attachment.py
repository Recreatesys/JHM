...
class IrAttachment(models.Model):
    ...
    def check_folder_id_permission(self):
        allowed_groups = [
            self.env.ref("documents_portal_management.group_document_user").id,
            self.env.ref("documents_portal_management.group_document_manager").id,
            self.env.ref("base.group_portal").id,
        ]
        if not any(group_id in self.env.user.groups_id.ids for group_id in allowed_groups):
    ...
    def action_attachment_send(self):
        ...
        # ✅ SAFER WAY to get compose form id
        compose_form_id = False
        try:
            compose_form_id = self.env.ref('mail.email_compose_message_wizard_form', False).id
        except Exception:
            compose_form_id = False

        # If still not found, fallback lookup
        if not compose_form_id:
            try:
                res = ir_model_data._xmlid_lookup('mail.email_compose_message_wizard_form')
                if len(res) > 2:
                    compose_form_id = res[2]
            except Exception:
                compose_form_id = False
        ...
        return {
            ...
            'views': [(compose_form_id, 'form')] if compose_form_id else [(False, 'form')],
